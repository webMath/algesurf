...
// GUI
	...
	JA.addHeader = function() {
		JA.header = document.body.appendChild( document.createElement( 'div' ) );
//		JA.menu.id = 'movable';
		JA.header.style.cssText = 'position: absolute; right: 20px; transition: right 1s; z-index: 10;';
		JA.header.innerHTML = '<h1>' +
			'<a href=JavaScript:slideMenu(); ><i class="fa fa-bars"></a></i>'+
		'</h1>';
	};

	function slideMenu(){
		JA.header.style.right = JA.header.style.right === '20px' ? '350px' : '20px';
// console.log( header.style.right );
	}


	JA.addMenu = function() {
		JA.menu = JA.header.appendChild( document.createElement( 'div' ) );
//		JA.menu.id = 'movable';
		JA.menu.title = 'Move this menu panel around the screen or iconize it';
//		JA.menu.addEventListener( 'mousedown', JA.mouseMove, false );

		JA.menu.style.cssText = 'background-color: #eee; height: ' + ( 0.95 * window.innerHeight ) + 'px; padding: 0 15px; overflow: auto; ' +
